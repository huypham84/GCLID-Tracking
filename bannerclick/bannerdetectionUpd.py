...
from libs.dbProcess import get_db_conn, insert_consent_banner, save_cookies_pg, save_full_network_data_AdsLanding
...
try:
    from . import cmpdetectionMOD as cd
except ImportError:
...
def run_chrome(driver, page_load_timeout=TIME_OUT):
...
def init(driver, headless=HEADLESS, input_file=None, num_browsers=NUM_BROWSERS, num_repetitions=1, domains_file=None, web_driver=None, v_db=None,
...
def open_domain_page(driver, domain, sleep=TEST_MODE_SLEEP):
    global this_url, this_domain, this_status
    mode = 1
    while True:
...
def open_domain_plus_detect_banner(driver,domain):
    open_domain_page(driver, domain)
    return detect_banners(driver)
...
def interact_with_banner(driver, banner_item, banners_data, choice, status, i, total_search=False, safari=False):
    ...
    try:
        ...
        if SIMPLE_DETECTION or choice != 2 or rej_flag:
            flag = click_func(el, ex_btns, file_name, SCREENSHOT, safari=safari)
            if not flag and NON_EXPLICIT:
                ...
                flag = click_func(el, nex_btns, file_name, SCREENSHOT, safari=safari)
                explicit_coeff = -1
...
def set_data_in_db(driver, data):
    global this_url, this_domain, this_status, this_lang, visit_db, this_run_url
    if data.openwpm:
...
def run_all(driver, dmns=None):   # this function is used for run the banner detection module only (Not through OpenWPM)
    global counter
    if dmns is None:
        dmns = get_domains()
...
# --- used ---
def run_ONE_pc_AdsCrawler(driver, choice, domain, url, timeoutLoad, headless = None, dir_bc = None):  # this function is used for run the banner detection module only (Not through OpenWPM)
    global counter
    init_ONE_pc(headless=headless, domain=domain, dir = dir_bc)
    res = None
    res = run_all_for_domain_AdsCrawler(driver, domain, url, choice, timeoutLoad)
...
def run_ONE_pc_nonDB(driver, choice, domain, url, timeoutLoad, headless = None, dir_bc = None, id_site=None, id_csv=None, country=None, run_no=None):  # this function is used for run the banner detection module only (Not through OpenWPM)
    global counter
    init_ONE_pc(headless=headless, domain=domain, dir = dir_bc)
    res = None
    res = run_all_for_domain_nonDB(driver, domain, url, choice, timeoutLoad, ID_SITE=id_site, ID_CSV=id_csv, COUNTRY=country, RUN_NO=run_no)
...
def run_ONE_pc(driver, id_site, choice, domain, headless = None, dir_bc = None):  # this function is used for run the banner detection module only (Not through OpenWPM)
    global counter
    ...
    try:
        url = open_domain_page(domain)
        res = run_all_for_domain(id_site, domain, url, choice)
...
def run_all_pc(driver,dmns=None):  # this function is used for run the banner detection module only (Not through OpenWPM)
    global counter
...
def run_all_for_domain_nonDB(driver, DMN, URL, CHOICE, T_OUTLOAD, ID_SITE=None, ID_CSV=None, COUNTRY=None, RUN_NO=None):
    global counter, SLEEP_TIME, this_domain
    
    driver.set_page_load_timeout(T_OUTLOAD)
    ...
    try:
        ...
        #if True:
        #if has_banner and ID_SITE is not None:
        if CHOICE == 0 or has_banner:
            ...
            with get_db_conn(COUNTRY, CHOICE) as conn:
                save_cookies_pg(conn, id_site=ID_SITE, id_csv=ID_CSV, country=COUNTRY, run_no=RUN_NO, this_domain=DMN, cookies=cookies, current_url=curr_url, after=False)
            
            print("Saved cookies data before interaction for domain: ", DMN)
            #print(f"[INFO] Collected {len(events)} network events before")
            #with get_db_conn() as conn:
            #    save_full_network_data_AdsLanding(conn, driver, ID_SITE, URL, events, after=False)
                #save_network_AdsLanding_data(conn, ID_SITE, URL, events, after=False)
            #print("Saved network data before interaction for domain: ", DMN)
            time.sleep(3)
        ...
        if CHOICE==0 or has_banner:
            ...
            with get_db_conn(COUNTRY, CHOICE) as conn:
                try:
                    save_cookies_pg(conn, id_site=ID_SITE, id_csv=ID_CSV, country=COUNTRY, run_no=RUN_NO, this_domain=DMN, cookies=cookies, current_url=curr_url, after=True)
                except Exception as e:
                    print("Err c:",e)
            ...
            with get_db_conn(COUNTRY, CHOICE) as conn:
                try:
                    save_full_network_data_AdsLanding(conn, driver, ID_SITE, ID_CSV, COUNTRY, RUN_NO, URL, events)
                except Exception as e:
                    print("Err: ",e)
            ...
            with get_db_conn(COUNTRY, CHOICE) as conn:
                try:
                    save_full_network_data_AdsLanding(conn, driver, ID_SITE, ID_CSV, COUNTRY, RUN_NO, URL, events, after=True)
                except Exception as e:
                    print("Err: ",e)
                    traceback.print_exc()
            print("Saved network data after interaction for domain: ", DMN)
        ...
        # #GET COOKIES AFTER INTERACTION
        # print("Getting cookies after interaction for domain: ", DMN)
        # time.sleep(15)
        # save_cookies_pg(driver, id_site=ID_SITE, this_domain=DMN, after=True)
        # time.sleep(1)

        # if GET_COOKIES:
        #     print("Getting cookies after interaction for domain: ", DMN)
        #     time.sleep(15)
        #     save_cookies(driver, after=True)
        #     time.sleep(1)
...
def run_all_for_domain_AdsCrawler(driver, DMN, URL, CHOICE, T_OUTLOAD):
    global counter, SLEEP_TIME, this_domain
...
def ReloadSafariPage(driver, id_site, path_site):
    driver.get(path_site)

    print(f"\n Reloading site: {id_site}")

    while(driver.current_url == path_site):
        el = driver.find_element(By.ID, str(id_site))
        print(el)
        el.click()
        time.sleep(3)  # Wait for the new page to load after clicking the ad link



def run_all_for_domain_SAFARI(driver, DMN, ADS_URL, URL, CHOICE, OUTPUT_FILE, T_OUTLOAD=30, ID_SITE=None, ID_CSV=None, COUNTRY=None, RUN_NO=None, safari = True):
    from libs.safariHelper import processing_cookie_dataSafari
    ...
    try:
        ...
        if BANNERCLICK:
            #print("Running banner detection for domain: ", DMN)
            banners = run_banner_detection(driver, Data, safari=safari)
            #print("Banners detected: ", len(banners))
            take_page_sc(driver, Data)
            Data.banners = banners
            Data.banners_data = extract_banners_data(driver, banners)
        ...
        if True:
            ...
            processing_cookie_dataSafari(id_site=ID_SITE, ads_url=ADS_URL, cookies=cookies_before, current_url=URL, output_file=OUTPUT_FILE, is_after=False, has_banner=has_banner)
        ...
        if BANNERINTERACTION:
            print("Running banner interaction for domain: ", DMN)
            interact_with_banners(driver, Data, CHOICE, safari=safari)
        ...
        if True:
            ...
            #cookies = driver.execute_cdp_cmd("Network.getAllCookies", {})
            cookies_after1 = driver.get_cookies()
            curr_url = driver.current_url
            processing_cookie_dataSafari(id_site=ID_SITE, ads_url=ADS_URL, cookies=cookies_after1, current_url=curr_url, output_file=OUTPUT_FILE, is_after=True, has_banner=has_banner)
            ...
            #cookies = driver.execute_cdp_cmd("Network.getAllCookies", {})
            cookies_after2 = driver.get_cookies()
            curr_url = driver.current_url
            processing_cookie_dataSafari(id_site=ID_SITE, ads_url=ADS_URL, cookies=cookies_after2, current_url=curr_url, output_file=OUTPUT_FILE, is_after=True, has_banner=has_banner)
...
def run_all_for_domain(ID_SITE, DMN, URL, CHOICE):
    global counter, SLEEP_TIME, this_domain
    this_domain = DMN
    has_banner = None
    try:
        ...
        domain_url = this_domain
        cmp_name = Data.nc_cmp_name if Data.nc_cmp_name else "unknown"
        user_choice = "accept" if CHOICE == 1 else "reject" if CHOICE == 2 else "unknown"
        ...
        print("ID Site: ", id_site)
        print("Domain_URL: ", domain_url)
        print("CMP Name: ", cmp_name)
        print("User Choice: ", user_choice)
        print("HTML: ", html_[:100] if html_ else "No HTML extracted")
        ...
        if has_banner:
            try:
                with get_db_conn() as conn:
                    insert_consent_banner(conn, id_site, domain_url, cmp_name, user_choice, html_, more_information="")
            except Exception as ex:
                print("Error inserting into database: ", ex)
...
def save_cookies(driver, after=False):
    global this_domain
    cookies = driver.execute_cdp_cmd("Network.getAllCookies", {})
...
def start_driver(user_data_dir = None, headless=False, extension_unpacked_folder=None, extension_crx_file=None):
    options = uc.ChromeOptions()
    options.add_argument("--no-first-run --no-service-autorun --password-store=basic")
    options.add_argument("--disable-notifications")
    options.add_argument("--disable-popup-blocking")
    ...
    if extension_unpacked_folder:
        if not os.path.isdir(extension_unpacked_folder):
            ...
            # Chrome needs this to load MV3 unpacked
            options.add_argument("--disable-extensions-except=" + extension_unpacked_folder)
            options.add_argument("--load-extension=" + extension_unpacked_folder)
    ...
    if extension_crx_file:
        if not os.path.isfile(extension_crx_file):
            raise SystemExit(f"CRX file not found: {extension_crx_file}")
        options.add_argument(f'--load-extension={extension_unpacked_folder}')
        #options.add_extension(extension_crx_file)
