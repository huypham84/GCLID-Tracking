...
from .bannerdetectionUpd import MyExceptionLogger
...
def has___tcfapiLocator(driver):
    locator_list = get___tcfapiLocator(driver)
    return bool(locator_list)


def get___tcfapiLocator(driver):
    return driver.find_elements(By.XPATH, "//iframe[contains(@name, '__tcfapiLocator')]")


def get_TCData(driver):
    TCData = None
    try:
...
def extract_CMP_list_from_server(driver):
...
def get_CMP_list(driver):
    global CMP_list
    if CMP_list is None:
        if EXTRACT_JSON:
            CMP_list = extract_CMP_list_from_url()
        else:
            CMP_list = extract_CMP_list_from_server(driver)
    return CMP_list
...
def get_CMP_id(driver):
    ping_return = get_pingReturn(driver)
    if ping_return:
        return ping_return.get("cmpId")
    return None


def get_CMP_name_by_id(driver, cmp_id):
    cmp_name = None
    cmps = get_CMP_list(driver)
    if EXTRACT_JSON:
        row = cmps.get(cmp_id)
        if row:
            cmp_name = row["name"]
...
def get_CMP_name(driver):
    cmp_id = get_CMP_id(driver)
    return get_CMP_name_by_id(driver, cmp_id)


def is_CMP_pv(driver, cmp_id):
    pv = False
    cmps = get_CMP_list(driver)
    if EXTRACT_JSON:
        row = cmps.get(cmp_id)
...
def detect_cmp(driver,CMP):
    CMP["__cmp"] = has___cmp(driver)
    CMP["__tcfapi"] = has___tcfapi(driver)
    CMP["__tcfapiLocator"] = has___tcfapiLocator(driver)
    CMP["cmp_id"] = get_CMP_id(driver)
    CMP["cmp_name"] = get_CMP_name_by_id(driver ,str(CMP["cmp_id"]))
    CMP["pv"] = is_CMP_pv(driver,str(CMP["cmp_id"]))


def run_cmp_detection(driver):
    CMP = {}
    try:
        detect_cmp(driver,CMP)
    except Exception as ex:
        pass
