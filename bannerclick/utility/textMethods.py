...
def find_els_contains_word(web_comp , word: str): # changing //*[contains(text(),"Cookies")] to //*[text()[contains(.,"Cookies")]]
    #TESTING - SAFARI - DOING
    print("SAFARI - LOG - 16")
    string = f"contains(string(.), '{word.capitalize()}') or contains(string(.), '{word.lower()}') or contains(string(.), '{word.upper()}')"
    return web_comp.find_elements(By.XPATH, f".//*[{string}]")
...
def to_xpath_text(string: str, safari=False):
    if safari:
        print("SAFARI - [LOG] TO XPATH TEXT")
        string = string.replace('contains(.,', 'contains(string(.),')
        string = f'.//*[{string}]'
    else:
        string = './/*[text()[' + string + ']]'
    return string
